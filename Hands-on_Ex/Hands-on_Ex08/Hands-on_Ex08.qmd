...
Use st_is_valid to check that the polygons are valid.

```{r}
st_is_valid(mpsz)
```

From the output, we notice that there are some invalid geometries within mpsz.

On top of setting the crs to 3414, we will use st_make_valid() from the sf package to
...
After importing, we can just take a look using glimpse() to better understand the data structure.
...
### Visualising relationships of independent variables
